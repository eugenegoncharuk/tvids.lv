...
class Requests extends Controller {
	...
	function show_request($id="") {
		...
		$items = $this->requests_items->get_items_by_pid($id, $cur_lang);

		// content_id may no longer resolve to a real tree node's content: for
		// bed-linen orders it's a bed_linen_image_leafs id instead, and for
		// old orders the tree node itself may since have been deleted. Fill
		// in the best name we can rather than leaving the row blank - the
		// price/count/sum are already safe, since those were snapshotted
		// onto requests_items at order time.
		foreach ($items as &$item) {
			if (empty($item['text'])) {
				$bl_leaf = $this->bedlinenimages->get_bed_linen_leaf($item['content_id']);
				$item['text'] = $bl_leaf ? htmlspecialchars($bl_leaf->name) : '(товар удалён, id: '.$item['content_id'].')';
			}
		}
		unset($item);
