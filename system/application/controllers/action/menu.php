...
class Menu extends Controller {
	...
	function show_addup($id) {
		...
		$data['node_type_options'] = $this->_node_type_options();
	...
	function show_edit($id) {
		...
		$data['page'] = 'admin/webmenu';
		$data['nav_button'] = 'menu';
		$data['node_type_options'] = $this->_node_type_options($node->node_type);

		$this->parser->parse('admin/container',$data);
	}

	function _node_type_options($current = 'text') {
		$options = array(
			'text' => 'Текст',
			'set_category' => 'Категория комплектов',
			'template_category' => 'Шаблон комплектов',
		);
		$html = '';
