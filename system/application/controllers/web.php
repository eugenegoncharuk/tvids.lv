...
class Web extends Controller {
	...
	function show($pid=1, $from=0 ,$msg_arr=''){
		// web/show/bedlinen/<id> - <id> is a bed_linen_image_leafs row id,
		// the stable public identifier for one (subcategory, pattern) pairing
		// now that the old per-pattern tree leaf nodes no longer exist.
		if ($pid === 'bedlinen') {
			$this->_show_bed_linen($from);
			return;
		}
		...
		// Комплекты") - there's nothing to show for the node itself, so send
		// visitors straight to its first subcategory (e.g. "Под.145x210,...").
		$node = $this->tree->find_by_id($pid);
		if ($node && $node->node_type == 'set_category'){
			$children = $this->bedlinenimages->get_child_categories($pid);
			if (!empty($children)){
				redirect('web/show/'.$children[0]->id);
			}
		}
		...
		$out = $this->_get_branch_by_pid_full(228,$cur_lang);
		$arr_pid = array();
		...
		if ($this->hier->is_node_under($pid, 228)){
			// Whether this node is a "subcategory grid" or a "single item" page
			// used to be decided purely from the tree (is_leaf: does anything
			// have this as a parent). That broke once the old per-pattern leaf
			// nodes were deleted: every curated subcategory now looks like a
			// leaf too (no children left), even though it should still show the
			// bed-linen grid. So curated subcategories are checked first and
			// always win, regardless of what the tree structure looks like now.
			$bl_grid_images = $this->bedlinenimages->get_images_for_subcat($pid);

			if (empty($bl_grid_images) && $this->hier->is_leaf($pid)){
				// If this leaf is one of the curated bed-linen patterns, show
				// its parent subcategory's template (edited via
				// action/text/show_text/<parent_id>) with {bed_linen_image}
				// swapped for this pattern's own uploaded photo, instead of
				// the leaf's own (usually empty) text.
				$bl_text = null;
				$bl_price = null;
				$node = $this->tree->find_by_id($pid);
				if ($node && $node->node_type == 'template_category'){
					$bl_image = $this->bedlinenimages->get_image_for_subcat_and_name($node->pid, $node->name);
					if ($bl_image){
						$bl_price = $bl_image->leaf_price;
						$tpl_q = $this->content->get_content($node->pid, $cur_lang, '2');
						if ($tpl_q->num_rows() > 0){
							// Same click-to-enlarge behaviour used elsewhere on the
							// site: any <a> inside #table_images is auto-bound to
							// fancybox (see main.php), so wrapping the thumbnail in
							// a link to the full-size photo is all that's needed.
							$image_html = '<div style="text-align:center;">'
								.'<table id="table_images" border="0" align="center" style="margin:0 auto;"><tbody><tr><td style="text-align:center;">'
								.'<a href="'.base_url().$bl_image->image_path.'"><img style="border:3px solid #cbecf9;" src="'.base_url().$bl_image->thumb_path.'" alt="" /></a>'
								.'</td></tr></tbody></table>'
								.'</div>';
							// Tag-tolerant: rich-text editors can split "{bed_linen_image}"
							// across separate <span> tags (e.g. "<span>{</span><span>bed_linen_image</span><span>}</span>").
							$bl_text = preg_replace('/\{(?:\s|<[^>]*>)*bed_linen_image(?:\s|<[^>]*>)*\}/i', $image_html, $tpl_q->row()->text);
						}
					}
				}
				...
				$res['text'] = ($bl_text !== null) ? $bl_text : @$q->row()->text;
				$res['content_id'] = $pid;
				$res['price'] = ($bl_price !== null) ? $bl_price : @$this->info_items->get_item($pid)->price;
				...
				$bl_images = $bl_grid_images;

				if (!empty($bl_images)) {
					// This subcategory has been curated in the Bed linen images
					// admin page - show those boxes instead of the raw per-leaf
					// grid below. Price is set per (image, subcategory) directly
					// in that admin page; each box links to its own stable
					// web/show/bedlinen/<leaf_row_id> page rather than a tree node.
					$rows = array();
					foreach ($bl_images as $bl_image) {
						$rows[] = array(
							'text' => '<img src="'.base_url().$bl_image->thumb_path.'" style="max-width:120px; max-height:105px;" alt="" />',
							// NOT "name": get_content_leafs_limit()'s rows use SELECT *
							// on nav_tree, so they incidentally carry their own "name"
							// column too - reusing that key here caused the legacy
							// grid's own embedded caption to double-render underneath it.
							'pattern_name' => '<strong>'.htmlspecialchars($bl_image->name).'</strong>',
							'price' => $bl_image->leaf_price,
							'content_id' => $pid,
							'view_url' => site_url().'web/show/bedlinen/'.$bl_image->leaf_row_id,
						);
					}
					$res = $this->lang->language;
					$res['rows'] = $rows;
	...
	// web/show/bedlinen/<id>: the public product page for one curated
	// (subcategory, pattern) pairing, identified by its own stable
	// bed_linen_image_leafs.id rather than a tree node (which may no
	// longer exist for this pattern under this subcategory).
	function _show_bed_linen($id) {
		$leaf = $this->bedlinenimages->get_bed_linen_leaf($id);
		if (!$leaf) {
			redirect('web/show/270');
		}
		...
		$out = $this->_get_branch_by_pid_full(228,$cur_lang);
		$arr_pid = array();
		$data['menu_text'] = $this->_print_tree_without_leafs($out, $arr_pid, $leaf->subcat_id);

		$out2 = $this->_get_branch_by_pid(229,$cur_lang);
		$arr_pid2 = array();
		$data['menu_text2'] = $this->_print_tree($out2, $arr_pid2, $leaf->subcat_id);

		$data['popular'] = $this->_get_popular_items();

		$q = $this->content->get_content($leaf->subcat_id,$cur_lang,'1');
		$data['selected_menu'] = ($q->num_rows()>0) ? $q->row()->text : $this->lang->line('no_data_for_category');

		// Same template + {bed_linen_image} substitution as the tree-leaf
		// version above, just sourced directly from the bed-linen tables.
		$tpl_q = $this->content->get_content($leaf->subcat_id, $cur_lang, '2');
		$text = '';
		if ($tpl_q->num_rows() > 0){
			$image_html = '<div style="text-align:center;">'
				.'<table id="table_images" border="0" align="center" style="margin:0 auto;"><tbody><tr><td style="text-align:center;">'
				.'<a href="'.base_url().$leaf->image_path.'"><img style="border:3px solid #cbecf9;" src="'.base_url().$leaf->thumb_path.'" alt="" /></a>'
				.'</td></tr></tbody></table>'
				.'</div>';
			$text = preg_replace('/\{(?:\s|<[^>]*>)*bed_linen_image(?:\s|<[^>]*>)*\}/i', $image_html, $tpl_q->row()->text);
		}

		$res = $this->lang->language;
		$res['text'] = $text;
		$res['content_id'] = $id;
		$res['price'] = $leaf->price;
		$res['buy_url'] = site_url().'web/buy/bedlinen/'.$id;
		$data['text'] = $this->parser->parse('item_view', $res, TRUE);

		$data['pid'] = $leaf->subcat_id;
		$data['lang'] = $cur_lang;
		$this->parser->parse('main',$data);
	}

	// web/buy/bedlinen/<id>: adds one unit of this pairing to the cart.
	// Cart entries for bed-linen items use a "bl_<id>" string key instead of
	// a plain tree id, so they can never collide with real tree node ids in
	// the same cart/session structures.
	function _buy_bed_linen($id) {
		$leaf = $this->bedlinenimages->get_bed_linen_leaf($id);
		if (!$leaf) {
			redirect('/web');
		}
		...
		if (!in_array($cart_id, @$cart['products'])){
			$cart['products'][] = $cart_id;
		}
		$cart['count'][$cart_id] = @$cart['count'][$cart_id] + 1;
		$cart['sum'][$cart_id] = @$cart['sum'][$cart_id] + $leaf->price;
		$cart['price'][$cart_id] = $leaf->price;
	...
	// Same as _get_branch_by_pid, but every descendant of a "Категория
	// комплектов" node always renders (subcategories and any remaining
	// leafs), instead of being silently dropped by _print_tree_without_leafs
	// whenever a child happens to be missing its own content/translation row.
	function _get_branch_by_pid_full($pid,$lang) {

		if (!$this->tree->exists_id($pid)){
			echo "Tree doesn't exist!";
		}

		$force_show_ids = array();
		foreach ($this->bedlinenimages->get_categories()->result() as $cat) {
			$force_show_ids[$cat->id] = true;
			$hier_q = $this->db->query('SELECT node_id FROM v2_nav_hier WHERE pid='.$cat->id);
			foreach ($hier_q->result() as $row) {
				$force_show_ids[$row->node_id] = true;
			}
		}

		$out = array();
		$t = $this->tree->get_branch_lang_all($pid,$lang);
		$ind = 0;
		while ($ind < $t->num_rows()){
			$r_arr = $t->row_array($ind);
			if ($ind < $t->num_rows() &&
			...
			if (isset($force_show_ids[$r_arr['id']])){
				$r_arr['branch']=1;
			}
			$out[] = $r_arr;
			$ind++;
	...
	function _get_popular_items(){
		...
		$q = $this->db->query("SELECT *, COUNT(hier.hier_pid) AS childs_count, 
			SUM(v2_info_items.clicks) as clicks,
			v2_content.pid AS content_id FROM v2_nav_tree 
			LEFT OUTER JOIN v2_content ON (v2_content.pid = v2_nav_tree.id) 
			LEFT OUTER JOIN v2_info_items ON (v2_content.pid = v2_info_items.pid) 
			LEFT OUTER JOIN (SELECT v2_nav_hier.pid as hier_pid FROM v2_nav_hier) hier 
			ON (hier.hier_pid=v2_nav_tree.id) WHERE v2_content.type='2' 
			AND v2_content.lang='".$this->lang->get_current_lang()."' AND v2_nav_tree.id IN (
			SELECT v2_nav_hier.node_id  as node_id FROM `v2_nav_hier` 
			WHERE v2_nav_hier.pid='201') GROUP BY v2_nav_tree.id 
	...
	// Price to use for cart/orders/request emails: bed-linen patterns
	// (node_type "template_category", i.e. "Шаблон комплектов") are priced
	// per (subcategory, pattern) in the Bed linen images admin page, not via
	// info_items - every regular ("Текст") item keeps using info_items as before.
	function _get_order_price($id) {
		if (is_string($id) && strpos($id, 'bl_') === 0) {
			$leaf = $this->bedlinenimages->get_bed_linen_leaf(substr($id, 3));
			return $leaf ? $leaf->price : 0;
		}
		$node = $this->tree->find_by_id($id);
		if ($node && $node->node_type == 'template_category') {
			$bl_image = $this->bedlinenimages->get_image_for_subcat_and_name($node->pid, $node->name);
			if ($bl_image) {
				return $bl_image->leaf_price;
			}
		}
		return @$this->info_items->get_item($id)->price;
	}

	/**
	 * We buy items here
	 */
    function buy($id, $bl_id=null) {
		// web/buy/bedlinen/<id>
		if ($id === 'bedlinen') {
			$this->_buy_bed_linen($bl_id);
			return;
		}
		...
		$price = $this->_get_order_price($id);
	...
	function _get_cart_items($cart){
		...
		foreach($cart_keys as $id) {
			...
			if (is_string($id) && strpos($id, 'bl_') === 0) {
				$leaf = $this->bedlinenimages->get_bed_linen_leaf(substr($id, 3));
				$text = $leaf ? htmlspecialchars($leaf->name) : '';
				$view_url = site_url().'web/show/bedlinen/'.substr($id, 3);
			} else {
				$q2 = $this->db->get_where('content', array("pid" => $id, "type" => 1, "lang" => $cur_lang), 1, 0);
				//echo $this->db->last_query();
				$text = $q2->row()->text;
				$view_url = site_url().'web/show/'.$id;
			}

			$items[] = array('pos'=>$pos, 'price'=>$price, 'text'=>$text,
							 'node_id'=>$id, 'view_url'=>$view_url, 'count'=>$cart['count'][$id],
	...
	function _add_requests_items_post($cart, $request_internal_id){
		...
		foreach($cart_keys as $id) {
			// content_id is a plain int column - bed-linen cart entries use a
			// "bl_<id>" string key in the session/cart only, so store just the
			// numeric bed_linen_image_leafs id here.
			$data3['pid'] = $request_internal_id;
			$data3['content_id'] = (is_string($id) && strpos($id, 'bl_') === 0) ? (int)substr($id, 3) : (int)$id;
	...
	function view_reg_info($login, $pass) {
		...
		$query = $this->db->query("SELECT * FROM v2_users, v2_info_users WHERE 
								(v2_users.name=\"".$login."\" AND v2_users.password=\"".$pass."\" 
								AND v2_users.id=v2_info_users.user_id)");
