...
class Request extends ActiveRecord {
	...
	function get_reqs_by_type($status=0, $from=0, $num_rows=30, $search_req="") {
		...
		if ($search_req!=""){
			$q = $this->db->query('SELECT *, reqs.id AS request_id, info.id as info_id FROM `v2_requests` as reqs, 
								`v2_info_users` as info
			...
			$q = $this->db->query('SELECT *, reqs.id AS request_id, info.id as info_id FROM `v2_requests` as reqs, `v2_info_users` as info
